# OpenSpec test fixtures

Deterministic, offline fixture harness shared by `agentic-ba` and `agentic-pm` tests (and any
other package that needs an OpenSpec-shaped target). It lives at the repo root, not inside
either package, because neither may depend on the other. Nothing here calls a real `openspec`
binary or a real `claude -p` engineer — see decisions D-A/D-B in `.agentic/plans/00-overview.md`.
Live-CLI acceptance is explicitly deferred.
...
`pm_fixture_setup` copies `target/` into a fresh temp dir per test (never mutates the
...
export into the calling shell because `WORK="$(pm_fixture_setup)"` runs the function in
