...
**Source docs: `docs/**/*.md`** at the target project root by default (the folder `agentic-init`
scaffolds) — unless the dispatch passes an explicit glob. `docs/` is human-authored input and is
**not** `openspec/specs/`, which is OpenSpec's own durable truth (I-11). If `docs/` is missing, stop
and say so rather than guessing a location.
...
- `openspec/changes/<id>/proposal.md` — starts with a frontmatter block emitted via
  `agentic-core/lib/frontmatter.sh <priority> [deps...]`; `<id>` comes from `lib/ba-changeid.sh "<title>"`.
