...
1. The target project has been bootstrapped with `/agentic-init` (installs the OpenSpec CLI,
   runs `openspec init` to create `openspec/`, and scaffolds `docs/`).
2. `agentic-ba` has read `docs/**/*.md` and produced changes with acceptance criteria +
   `depends_on`/`priority` frontmatter.
