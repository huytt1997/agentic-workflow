...
WORK="$(pm_fixture_setup "$DIR")"; BARE="$(mktemp -d)"
...
out="$(AGENTIC_PROJECT_ROOT="$WORK" PATH="$DIR/fixtures/bin:$PATH" bash "$DIR/../bin/pm-run.sh")"
rc=$?
...
out2="$(AGENTIC_PROJECT_ROOT="$BARE" PATH="$DIR/fixtures/bin:$PATH" bash "$DIR/../bin/pm-run.sh" 2>/dev/null)"
rc2=$?
