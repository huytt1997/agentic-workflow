...
| **D-2**  | Five packages: `agentic-core` + `agentic-engineer` + `agentic-init` + `agentic-ba` + `agentic-pm`; core is shared | Clean seam between the enforcement kernel and the role pipelines. **Amended 2026-07-15:** `agentic-management` split into `agentic-ba` + `agentic-pm` so each installs independently; the BA↔PM frontmatter contract moved into core, the only package both depend on. |
...
| **D-13** | Install/update/uninstall via `packages/*.sh` with `--target` (default `~/.claude`); **copy-only** | **Amended 2026-07-15:** symlink mode removed; `--mode symlink` now errors. Live-editable installs are gone — the dev loop uses `claude --plugin-dir packages/<name>`. |
